...
// APPROVE KARANTINA → KASIR (BRANCH)
...
async function submitApprove() {
  ...
  try {
    ...
    toast.success(`${qty} pcs berhasil disetujui dan masuk ke stok kasir!`)
    showApproveModal.value = false
    fetchData()
  } catch (err) {
    toast.error(err.response?.data?.message || 'Gagal menyetujui stok.')
  } finally {
    approveSaving.value = false
  }
}

// ─── TRANSFER STOK DRAWER CONTROL ───────────────────────────────────────────const showTransferDrawer = ref(false)
                ...
                <tbody>
                  <tr v-for="b in paginatedBalances" :key="b.id" class="border-b border-zinc-100 dark:border-zinc-800/60 hover:bg-zinc-50/80 dark:hover:bg-zinc-900/40 transition-colors">
                    ...
                    <td class="py-4">
                      <div class="flex flex-col gap-1">
                        <div class="flex items-center gap-1.5 font-medium"
                          :class="b.locationType?.toUpperCase() === 'QUARANTINE'
                            ? 'text-red-600 dark:text-red-400'
                            : 'text-zinc-700 dark:text-zinc-300'">
                          <AlertTriangle v-if="b.locationType?.toUpperCase() === 'QUARANTINE'" class="h-3.5 w-3.5 opacity-70" />
                          <Warehouse v-else-if="b.locationType === 'warehouse'" class="h-3.5 w-3.5 opacity-50" />
                          <Building2 v-else class="h-3.5 w-3.5 opacity-50" />
                          {{ b.locationType?.toUpperCase() === 'QUARANTINE' ? 'Karantina' : getLocationName(b.locationType, b.locationId) }}
                        </div>
                        <Badge variant="outline"
                          :class="['w-fit text-[9px] px-1.5 py-0 uppercase tracking-widest font-bold',
                            b.locationType?.toUpperCase() === 'QUARANTINE'
                              ? 'border-red-300 text-red-600 dark:border-red-700 dark:text-red-400'
                              : '']">
                          {{ b.locationType?.toUpperCase() === 'QUARANTINE' ? 'Karantina' : b.locationType }}
                        </Badge>
                      </div>
                    </td>
                    ...
                    <td class="pr-5 py-4 text-right">
                      <Button v-if="b.locationType?.toUpperCase() === 'QUARANTINE'"
                        variant="ghost" size="sm"
                        class="h-8 px-3 text-[11px] font-bold text-red-500 hover:text-red-700 hover:bg-red-50 dark:hover:bg-red-900/20 gap-1.5"
                        @click="openDispose(b)">
                        <Trash2 class="h-3.5 w-3.5" />
                        Dispose
                      </Button>
                      <Button v-if="b.locationType?.toUpperCase() === 'QUARANTINE' && can('stock_balance.update') && !isSuperAdmin"
                        variant="ghost" size="sm"
                        class="h-8 px-3 text-[11px] font-bold text-emerald-600 hover:text-emerald-700 hover:bg-emerald-50 dark:hover:bg-emerald-900/20 gap-1.5"
                        @click="openApprove(b)">
                        <CheckCircle2 class="h-3.5 w-3.5" />
                        Approve ke Kasir
                      </Button>
                      <Button v-if="b.locationType?.toUpperCase() !== 'QUARANTINE'" variant="ghost" size="icon" class="h-8 w-8 text-zinc-400 hover:text-primary transition-colors">
            ...
            <div class="px-6 py-5 space-y-4">
              ...
              <div class="space-y-1.5">
                <label class="text-[11px] font-bold text-zinc-500 uppercase tracking-wider">
                  Jumlah Dispose <span class="text-destructive normal-case font-normal">*</span>
                </label>
                <div class="relative">
                  <input
                    v-model="disposeQty"
                ...
                <button
                  type="button"
                  @click="disposeQty = disposeTarget.qty.toString()"
              ...
              <div class="space-y-1.5">
                ...
                <input
                  ...
                  placeholder="cth: dikembalikan ke supplier karena expired"
          ...
          <div class="bg-white dark:bg-zinc-900 rounded-[24px] shadow-2xl w-full max-w-sm border border-emerald-200 dark:border-emerald-800/40 pointer-events-auto overflow-hidden">
            ...
            <div class="flex items-center gap-3 px-6 py-4 border-b border-emerald-100 dark:border-emerald-900/40 bg-emerald-50/50 dark:bg-emerald-950/20">
              ...
              <div>
                ...
                <p class="text-[11px] text-muted-foreground mt-0.5">Pindahkan stok dari karantina ke saldo aktif cabang</p>
              </div>
            ...
            <div class="px-6 py-5 space-y-4">
              ...
              <div class="p-3 rounded-xl bg-zinc-50 dark:bg-zinc-800/50 border border-zinc-100 dark:border-zinc-800">
                ...
                <div class="flex items-center justify-between mt-2 pt-2 border-t border-zinc-100 dark:border-zinc-800">
                  <span class="text-[11px] text-zinc-500">Stok Karantina (dari PO)</span>
                ...
                <div class="flex items-center justify-between mt-1">
                  ...
                  <span class="text-[11px] font-bold text-zinc-700 dark:text-zinc-300">{{ getLocationName('branch', approveTarget.locationId) }}</span>
                </div>
              </div>
              ...
              <div class="space-y-1.5">
                <label class="text-[11px] font-bold text-zinc-500 uppercase tracking-wider">
                  Jumlah yang Disetujui <span class="text-destructive normal-case font-normal">*</span>
                </label>
                <div class="relative">
                  <input
                    v-model="approveQty"
              ...
              <!-- Keterangan -->
              <div class="space-y-1.5">
                <label class="text-[11px] font-bold text-zinc-500 uppercase tracking-wider">Catatan <span class="normal-case font-normal">(opsional)</span></label>
                <input
                  v-model="approveNotes"
                  placeholder="cth: barang sudah dicek, kondisi baik"
