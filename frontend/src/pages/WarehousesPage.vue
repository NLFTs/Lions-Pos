...
import {
  ...
  MapPin, Warehouse, Shield, Check, Users
...
const isAllSelected = computed(() => {
  const visible = paginatedWarehouses.value
  return visible.length > 0 && visible.every(w => selectedIds.value.includes(w.id))
})
...
function toggleSelectAll() {
  if (isAllSelected.value) {
    ...
    paginatedWarehouses.value.forEach(w => {
      if (!selectedIds.value.includes(w.id)) selectedIds.value.push(w.id)
    })
  }
}
...
async function bulkDelete() {
  ...
  const ok = await confirm({
    ...
    description: `Hapus ${count} gudang secara permanen? Data stok di gudang ini mungkin terpengaruh.`,
    confirmLabel: 'Hapus',
    cancelLabel: 'Batal',
  })
  if (!ok) return
...
const formErrors = ref({ name: '', password: '' })

function openCreate() {
  form.value = emptyForm()
  formErrors.value = { name: '', password: '' }
...
function openEdit(w) {
  form.value = { id: w.id, name: w.name, address: w.address || '', username: '', password: '', roleIds: [] }
  formErrors.value = { name: '', password: '' }
  formError.value = null
...
function validate() {
  formErrors.value = { name: '', password: '' }
  let ok = true
  if (!form.value.name.trim()) { formErrors.value.name = 'Nama gudang wajib diisi'; ok = false }
...
async function saveWarehouse() {
  ...
  try {
    const payload = {
      ...
      ...(modalMode.value === 'create' && form.value.username?.trim() && {
    ...
    formError.value = err.response?.data?.message || 'Gagal menyimpan gudang.'
  } finally {
    saving.value = false
  }
}
...
function doDelete(w) { deleteModal.value = { show: true, warehouse: w, confirmText: '' } }
      ...
      <Transition name="fade" mode="out-in">
        ...
        <div v-if="!showDrawer" key="table" class="flex flex-col gap-6">
          ...
          <div class="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
            <div>
              ...
              <p class="text-muted-foreground text-sm mt-1">Kelola data gudang beserta akun penggunanya.</p>
            </div>
            <div class="flex flex-col sm:flex-row items-center gap-3 w-full md:w-auto">
              <div class="w-full sm:w-72">
                <DataTableSearch v-model="searchQuery" placeholder="Cari gudang..." />
              </div>
              <Button
                v-if="can('warehouse.store') && !isSuperAdmin"
                @click="openCreate"
                size="sm"
            ...
            <CardContent class="p-0">
              ...
              <div v-else-if="filteredWarehouses.length === 0" class="flex flex-col items-center justify-center py-24 text-muted-foreground">
                <div class="w-14 h-14 rounded-full bg-muted flex items-center justify-center mb-4">
                  <Warehouse class="h-7 w-7 opacity-40" />
                </div>
                <p class="text-sm font-medium">Belum ada data gudang</p>
                <p class="text-xs text-muted-foreground/70 mt-1">Mulai dengan menambahkan gudang pertama Anda.</p>
                <Button v-if="can('warehouse.store') && !isSuperAdmin" size="sm" class="mt-4" @click="openCreate">
                  <Plus class="h-3.5 w-3.5 mr-1.5" /> Tambah Gudang
                </Button>
              </div>
              ...
              <div v-else>
                ...
                <Transition name="fade">
                  <div v-if="selectedIds.length > 0" class="flex items-center justify-between px-5 py-3 bg-primary/5 border-b border-border">
                    ...
                    <Button v-if="can('warehouse.delete')" size="sm" variant="destructive" class="h-8 text-xs gap-1.5" @click="bulkDelete">
                ...
                <div class="md:hidden flex flex-col divide-y divide-zinc-100 dark:divide-zinc-800/60">
                  <div v-for="w in paginatedWarehouses" :key="w.id" class="p-4 flex flex-col gap-3">
                    <div class="flex items-start justify-between">
                      <div class="flex items-center gap-3">
                        <div class="w-10 h-10 rounded-lg flex items-center justify-center font-bold text-sm shrink-0 border border-zinc-200/50 dark:border-zinc-800/50" :style="avatarStyle(w.name)">
                          {{ w.name?.charAt(0).toUpperCase() }}
                        </div>
                        <div>
                          <p class="font-semibold text-sm text-zinc-900 dark:text-zinc-100">{{ w.name }}</p>
                          <p v-if="w.address" class="text-xs text-zinc-400 truncate max-w-[160px]">{{ w.address }}</p>
                        </div>
                      </div>
                      <div class="flex gap-1 shrink-0">
                        <Button v-if="can('warehouse.update') && !isSuperAdmin" variant="ghost" size="icon" class="h-8 w-8" @click="openEdit(w)">
                          <Pencil class="h-3.5 w-3.5" />
                        </Button>
                        <Button v-if="can('warehouse.delete') && !isSuperAdmin" variant="ghost" size="icon" class="h-8 w-8 text-zinc-400 hover:text-destructive" @click="doDelete(w)">
                ...
                <div class="hidden md:block overflow-x-auto">
                  <table class="w-full text-sm">
                    ...
                    <tbody>
                      <tr
                        v-for="w in paginatedWarehouses"
                        :key="w.id"
                        ...
                        @click="can('warehouse.update') && !isSuperAdmin && openEdit(w)"
                      >
                        <td class="w-12 pl-5 py-3" @click.stop>
                          <input type="checkbox" class="rounded border-zinc-300 dark:border-zinc-700 text-primary focus:ring-primary h-4 w-4 cursor-pointer" :checked="selectedIds.includes(w.id)" @change="toggleSelect(w.id)" />
                        </td>
                        <td class="pl-2 py-3">
                          <div class="flex items-center gap-3">
                            <div class="w-9 h-9 rounded-lg flex items-center justify-center font-bold text-sm shrink-0 border border-zinc-200/50 dark:border-zinc-800/50" :style="avatarStyle(w.name)">
                              {{ w.name?.charAt(0).toUpperCase() }}
                            </div>
                            <div>
                              <p class="font-semibold text-zinc-900 dark:text-zinc-100 text-[13px] leading-tight">{{ w.name }}</p>
                              <p class="text-[10px] text-zinc-400 mt-0.5">ID #{{ w.id }}</p>
                            </div>
                          </div>
                        </td>
                        <td class="py-3 max-w-[320px]">
                          <div v-if="w.address" class="flex items-center gap-1.5 text-xs text-zinc-500">
                        ...
                        <td class="pr-5 py-3 text-right" @click.stop>
                          <div class="flex justify-end gap-1">
                            <Button
                              v-if="can('warehouse.update') && !isSuperAdmin"
                            ...
                            <Button
                              v-if="can('warehouse.delete') && !isSuperAdmin"
                              variant="ghost" size="icon"
                ...
                <DataTablePagination
                  v-if="filteredWarehouses.length > 0 && !loading"
                  :page="page"
                  :page-size="pageSize"
        ...
        <div v-else key="form" class="flex flex-col gap-6 max-w-2xl">
          ...
          <div class="flex items-center gap-4">
            ...
            <div>
              ...
              <p class="text-muted-foreground text-sm mt-0.5">
                {{ modalMode === 'create' ? 'Buat gudang baru, akun pengelola bersifat opsional.' : 'Perbarui informasi gudang.' }}
              </p>
            </div>
          </div>
          ...
          <Card class="border-zinc-200 dark:border-zinc-800 shadow-sm">
            <CardContent class="p-6 space-y-6">
              ...
              <div class="space-y-1.5">
                <Label for="wh-name">Nama Gudang <span class="text-destructive">*</span></Label>
                <Input id="wh-name" v-model="form.name" placeholder="Contoh: Gudang Sentral Jakarta" :disabled="saving" />
                <p v-if="formErrors.name" class="text-xs text-destructive mt-1">{{ formErrors.name }}</p>
              </div>

              <!-- Alamat -->
              <div class="space-y-1.5">
                <Label for="wh-address">Alamat</Label>
                <textarea
                  id="wh-address"
              ...
              <!-- Akun Pengelola (opsional, hanya saat create) -->
              <template v-if="modalMode === 'create'">
                <div class="border-t border-border pt-6">
                  <div class="flex items-center gap-2 mb-4">
                    ...
                    <div>
                      <p class="text-sm font-semibold text-foreground">Akun Pengelola Gudang <span class="text-muted-foreground font-normal text-xs">(opsional)</span></p>
                      <p class="text-xs text-muted-foreground">Kosongkan username jika tidak perlu akun tersendiri.</p>
                    </div>
                  </div>
                  <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div class="space-y-1.5">
                      <Label for="wh-username">Username</Label>
                      <Input id="wh-username" v-model="form.username" placeholder="username_gudang" :disabled="saving" autocomplete="off" />
                    </div>
                    <div class="space-y-1.5">
                      <Label for="wh-password">Password</Label>
                      <Input id="wh-password" type="password" v-model="form.password" placeholder="Min. 6 karakter" :disabled="saving" autocomplete="new-password" />
                      <p v-if="formErrors.password" class="text-xs text-destructive">{{ formErrors.password }}</p>
                    </div>
                  </div>
                </div>

                <!-- Assign Roles (hanya jika username diisi) -->
                <Transition name="fade">
                  <div v-if="form.username?.trim()" class="space-y-2">
                    ...
                    <p v-if="form.username?.trim() && form.roleIds.length === 0" class="text-xs text-destructive">
                      Wajib pilih minimal satu role jika username diisi.
                    </p>
                  </div>
                </Transition>
              </template>
          ...
          <div class="flex justify-end gap-3">
            ...
            <Button @click="saveWarehouse" :disabled="saving" class="bg-primary hover:bg-primary/90 min-w-[120px]">
              <Loader2 v-if="saving" class="h-4 w-4 mr-2 animate-spin" />
              {{ modalMode === 'create' ? 'Simpan Gudang' : 'Perbarui' }}
            </Button>
          </div>
        </div>
        ...
        <div v-if="deleteModal.show" class="fixed inset-0 z-[60] flex items-center justify-center p-4 pointer-events-none">
          <div class="bg-card rounded-xl shadow-2xl w-full max-w-md border border-border pointer-events-auto p-6">
            ...
            <p class="text-sm text-muted-foreground mb-4 leading-relaxed">
              Anda akan menghapus <span class="font-bold text-foreground">{{ deleteModal.warehouse?.name }}</span> secara permanen. Data stok di gudang ini mungkin terpengaruh.
            </p>
            <div class="space-y-2 mb-6">
              <Label class="text-xs text-muted-foreground">Ketik nama gudang untuk konfirmasi:</Label>
              <Input
                v-model="deleteModal.confirmText"
                :placeholder="deleteModal.warehouse?.name"
            ...
            <div class="flex justify-end gap-3">
              ...
              <Button
                variant="destructive"
                :disabled="deleteModal.confirmText !== deleteModal.warehouse?.name || deleting"
                @click="confirmDelete"
              >
